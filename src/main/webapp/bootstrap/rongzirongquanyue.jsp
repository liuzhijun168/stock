...
<%
	String sql = "select sdq.*,used,rongquan.create_date cd from stock_data_query sdq,rongquan where rongquan.code=sdq.b and sdq.o!=0 and rongquan.used>=500";
						  ...
						  <tbody>
						   <%
							  while(resultSet.next()){
								  ...
								  float m30 = resultSet.getFloat("m30");
								  float m60 = resultSet.getFloat("m60");
								  float m120 = resultSet.getFloat("m120");
								  float m250 = resultSet.getFloat("m250");
								  float e = resultSet.getFloat("e");
								  if(e == -100){
									  continue;
								  }
								  //t振幅
								  String t = String.format("%.2f",((d - s) * 100 / s)-((q - s) * 100 / s));
								  String createDate = resultSet.getString("cd").substring(5);
								  %>
								<tr>
								<td><%=b%></td>
								<td><%=resultSet.getString("c")+"("+d+")" %></td>
								<td><%=resultSet.getString("g") %></td>
								<td><%=resultSet.getString("used") %></td>
								<td><%=e%></td>
								<td><%=t %></td>
								<td><%=d<m5?"m5;":"" %><%=d<m10?"m10;":"" %><%=d<m20?"m20;":"" %><%=d<m30?"m30;":"" %><%=d<m60?"m60;":"" %><%=d<m120?"m120;":"" %><%=d<m250?"m250;":"" %></td>
